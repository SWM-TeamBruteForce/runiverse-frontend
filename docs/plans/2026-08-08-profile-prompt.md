...
- [x] **Step 3: 에뮬레이터에서 확인한다** — 2026-08-11 **7개 전부 통과**

> `isOnboarded=false` 계정이 없어 오래 막혀 있었다. 카카오 로그인이 붙으면서
> **서버가 계정을 새로 만들어 주는 경로**가 생겨 풀렸다.
>
> 덤으로 확인된 것 — 카카오 로그인 → 계정 생성(`isOnboarded=false`) ·
> 자동 로그인(`POST /auth/refresh → 200`) · 페이스 [건너뛰기] → "측정 전" →
> 온보딩 제출(`201`, PR #16의 720초 치환 경로).
>
> **6·7번이 핵심이었고 둘 다 통과했다.** 특히 7번(앱 재시작 후에도 카드 없음)이
> `markOnboarded`가 상태와 저장소를 함께 켠다는 증거다.
