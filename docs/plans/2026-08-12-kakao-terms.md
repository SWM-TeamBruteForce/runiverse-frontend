...
## 에뮬레이터 확인 결과 (2026-08-13)

앱을 지웠다 새로 설치하고 시작했다. `topResumedActivity`를 함께 봤다 —
**화면 캡처만으로는 브라우저가 함께 떴는지 알 수 없어서다.**

| # | 조작 | 기대 | 결과 |
|---|---|---|---|
| 1 | 새로 설치 → [카카오로 계속하기] | 약관 화면, 카카오가 열리지 않는다 | ✅ `topResumedActivity`가 `MainActivity` 그대로 |
| 2 | 마케팅 항목 | `선택` 배지 · 안 켜도 CTA가 열린다 | ✅ 필수 3개만 켠 상태에서 CTA 활성. 전체 동의는 꺼진 채 남았다 |
| 3 | 동의하고 계속 | 카카오 인가 → 홈 | ⚠️ **인가까지 확인**(`CustomTabActivity` + 카카오 로그인 페이지). 홈까지는 못 봤다 — 아래 참조 |
| 4 | 다시 카카오 | 약관 없이 곧바로 인가 | ✅ 약관을 거치지 않고 2초 만에 카카오 |
| 5 | 앱 데이터 삭제 후 다시 | 약관이 다시 뜬다 | ✅ 알려진 한계 그대로 |
| + | 이메일 가입 경로 | 약관 → 가입 화면 | ✅ `TermsNext` 기본값이 유지된다 |

배지 색도 눈으로 갈렸다 — `필수`는 primary, `선택`은 회색이다.

### 3번을 끝까지 못 본 이유

**테스트 서버의 애플리케이션이 떠 있지 않다.** 앱 서버 호스트는 살아 있지만
(SSH만 열림) 8080을 포함해 서비스 포트가 전부 닫혀 있었다. bastion 키로는 앱
서버에 로그인할 수 없어(`Permission denied`) 켤 수도 없었다.

다만 **이 PR이 바꾼 부분은 인가보다 앞에 있다.** 인가 이후(코드를 서버에 넘기고
홈으로 가는 길)는 손대지 않았고 `sign_in_page_test.dart`가 그대로 지킨다.
서버가 뜨면 3번의 나머지 절반만 다시 보면 된다.
