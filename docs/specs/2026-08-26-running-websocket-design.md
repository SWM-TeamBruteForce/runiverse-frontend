...
10초마다  _lastSent 이후 5행을 읽어 전송 → _lastSent 갱신
재연결    _lastSent - 겹침(15점 ≈ 30초) 부터 다시 보낸다
...
그래서 `_lastSent`에서 **30초(15점)를 물러서서** 다시 보낸다. 서버가
`sequence`로 중복을 거르므로 겹치는 것은 공짜고, 전송량은 900점이 아니라 15점이다.
