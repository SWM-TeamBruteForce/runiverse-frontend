...
enum AuthFailure {
  ...
  /// 네트워크에 닿지 못했다. **요청은 서버에 가지 않았다.**
  ///
  /// 연결 자체가 안 됐거나 인증서가 거절된 경우다. 서버가 아무것도 받지
  /// 않았으므로 **같은 요청을 다시 보내도 안전하다.**
  network,

  /// 보냈는지 **알 수 없다.** 응답을 기다리다 시간이 초과됐다.
  ///
  /// ## ⚠️ [network]와 반드시 나눈다
  ///
  /// 이쪽은 **서버가 이미 처리했을 수 있다.** 갱신은 refresh 토큰을 회전시키므로,
  /// 같은 토큰으로 다시 보내면 서버는 이미 쓴 토큰을 보고 401을 준다 —
  /// 재시도가 성공을 실패로 바꾼다(에뮬레이터에서 두 계정이 이렇게 로그아웃됐다).
  ///
  /// 신청·취소처럼 몸통이 있는 요청도 마찬가지다. 다시 보내면 중복이 된다.
  /// **"실패했다"가 아니라 "모른다"로 다뤄야 한다** — 재시도 대신 상태를 다시 읽는다.
  deliveryUnknown,
