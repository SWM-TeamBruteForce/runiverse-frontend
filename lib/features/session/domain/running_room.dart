...
enum RunningRoomFailure {
  ...
  /// 서버가 **진행 중인 것이 없다**고 답했다(`GET /users/me/status` = `IDLE`).
  ///
  /// 러닝을 붙들고 있는데 서버는 이미 끝냈다는 뜻이다 — 강제 종료됐거나 다른
  /// 기기에서 끝냈다. 계속 좌표를 올려 봐야 서버는 받지 않는다.
  ///
  /// ⚠️ **로컬 트랙은 지우지 않는다.** 종료 확인을 못 받았으므로 서버가
  /// 받았는지 알 수 없고, 지우면 되돌릴 수 없다.
  endedByServer,
