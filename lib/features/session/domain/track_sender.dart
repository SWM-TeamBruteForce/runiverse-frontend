...
///
/// ## ⚠️ 이것은 명세에서 의도적으로 벗어난 자리다
///
/// 복구 가이드 5절은 "첫 `sequence`부터 전체 재전송"이다. 전체로 되돌리면
/// 밀린 것이 수백 점일 때 큰 배치가 연달아 나가 close code **1009**로 끊기고,
/// 끊기면 다시 전체를 보내며 **스스로 증폭한다** — 실제로 겪었다.
///
/// 앱이 죽었다 살아난 경우는 이 객체가 새로 태어나 [_lastSent]가 0이므로
/// **그때는 명세대로 전체를 다시 보낸다.** 여기서 줄이는 것은 소켓만 끊겼다
/// 붙는 경우뿐이고, 그때는 넘긴 것이 도착했다고 볼 근거가 있다.
///
/// 서버의 WS 한도·수신 버퍼 설정이 확인되면 다시 재어야 한다.
class TrackSender {
